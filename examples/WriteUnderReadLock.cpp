 ...
 *
 * By using the software, you accept these terms. If you do not accept them, do not use the software. Instead, contact us at http://runtimeverification.com/support.
 *
 * 1. OVERVIEW.
 ...
 *
 * 5. SUPPORT SERVICES. RV provides limitted support services for the software. Please check our website and/or contact us at http://runtimeverification.com/support.
 *
...
struct ReadWriteLock {
    shared_mutex mLock;
    shared_lock< shared_mutex > mReadLock;
    unique_lock< shared_mutex > mWriteLock;

    ReadWriteLock() {
        mReadLock = shared_lock< shared_mutex > (mLock);
        mWriteLock = unique_lock< shared_mutex > (mLock);
    }

    shared_lock< shared_mutex > &readLock() {
        return mReadLock;
    }
    unique_lock< shared_mutex > &writeLock() {
        return mWriteLock;
    }
};
